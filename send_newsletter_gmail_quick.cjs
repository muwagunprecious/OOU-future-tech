...
async function sendQuickGmail() {
    const recipients = [
        { email: 'temytopsyebenezerfresh@gmail.com', name: 'Ebenezer' },
        { email: 'omotoyosiagboola74@gmail.com', name: 'Omotoyosi' },
        { email: 'professorprecious03@gmail.com', name: 'Precious' }
    ];

    console.log('Sending newsletter via Gmail SMTP to quick recipients...');
    
    for (let r of recipients) {
        try {
            const html = htmlTemplate.replace('RECIPIENT_NAME', r.name);
            const mailOptions = {
            ...
            console.log(`✅ Successfully sent via Gmail to ${r.email}: ${info.messageId}`);
        } catch (err) {
