...
if (!fs.existsSync(recipientsPath)) {
    console.error('❌ recipients.json not found! Please run prepare_recipients.cjs first.');
    process.exit(1);
}

// Load and parse recipients
let recipientsData = JSON.parse(fs.readFileSync(recipientsPath, 'utf8'));

// Initialize status fields if not present
let initialized = false;
recipientsData = recipientsData.map(r => {
    if (!r.status) {
        r.status = 'pending';
        initialized = true;
    }
...
if (initialized) {
    fs.writeFileSync(recipientsPath, JSON.stringify(recipientsData, null, 2), 'utf8');
}
...
async function sendBulk() {
    console.log('🏁 Starting bulk newsletter process...');
    
    // Find first 400 pending recipients
    const pending = recipientsData.filter(r => r.status === 'pending');
    console.log(`✉️ Found ${pending.length} pending recipients out of ${recipientsData.length} total.`);

    if (pending.length === 0) {
        console.log('✅ All newsletters have already been successfully dispatched!');
        return;
    }

    const batchSize = pending.length; 
    const batch = pending;
    console.log(`🚀 Dispatching all ${batchSize} pending emails now...`);
    ...
    for (let i = 0; i < batch.length; i++) {
        ...
        const personalizedHtml = baseHtmlContent.replace('RECIPIENT_NAME', recipient.name || 'FutureTech Family');

        const mailOptions = {
            from: `"Future Tech Academy (FTA)" <${env.EMAIL_USER}>`,
            to: recipient.email,
        ...
        try {
            ...
            successCount++;
            console.log(`[${i+1}/${batchSize}] ✅ Sent successfully to: ${recipient.email}`);
        } catch (err) {
            recipient.status = 'failed';
            recipient.error = err.message;
            failCount++;
            console.error(`[${i+1}/${batchSize}] ❌ Failed sending to: ${recipient.email}. Error: ${err.message}`);
        }

        // Save progress to recipients.json periodically
        if ((i + 1) % 10 === 0 || i === batch.length - 1) {
            fs.writeFileSync(recipientsPath, JSON.stringify(recipientsData, null, 2), 'utf8');
            console.log('💾 Progress checkpoint saved.');
        }

        // Polite sleep (1.8 seconds) to prevent Gmail spam filter blocks
        await sleep(1800);
    }
    ...
    const remaining = recipientsData.filter(r => r.status === 'pending').length;
    console.log(`- Remaining Pending Recipients for next batch: ${remaining}`);
}
